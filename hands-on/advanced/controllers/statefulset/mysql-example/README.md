...
* [English] https://kubernetes.io/docs/tasks/run-application/run-replicated-stateful-application/
* [Chinese] https://kubernetes.io/zh-cn/docs/tasks/run-application/run-replicated-stateful-application/

<!-- overview -->

This page shows how to run a replicated stateful application using a statefulset.
...
The headless Service provides a home for the DNS entries that the StatefulSet controllers creates for each Pod that's part of the set.
...
Combined with the StatefulSet controller's deployment order guarantee, this ensures the primary MySQL server is Ready before creating replicas, so they can begin
replicating.
